...
|---|---|---|---|---|
| Llama 3.1 Instruct 1B | 1 B | ~ 2.3 GB (FP16) | ~ 2.3 GB | Molto basso. Ideale per inferenze leggere su GPU o CPU di base. |
...
| Llama 3.1 Instruct 70B | 80 B | ~ 40 GB (Q4_K_M) / 141 GB (FP16) | ~ 40-141 GB | Medio/Alto. Richiede GPU di fascia alta o più GPU. |
...
Un <a href="https://github.com/federicoibba/nutritional-information-rag/blob/main/services/qwen.py" target="_blank">servizio Qwen</a>  è stato rilasciato su <a href="https://modal.com" target="_blank">modal</a> e può essere testato utilizzando:
...
### Servizio Llama
È stato implementato anche un <a href="https://github.com/federicoibba/nutritional-information-rag/blob/main/services/llama.py" target="_blank">servizio Llama</a> che può essere testato come di seguito:
...
```md
Si prega di utilizzare solo il seguente contesto per rispondere alla domanda.
**Regola di precedenza: scegli sempre i dati nutrizionali per gli alimenti CRUDI, se disponibili.**

Ottieni i dati nutrizionali del seguente ingrediente alimentare: **Salmone**.
OPZIONI DI CONTESTO:
nome del prodotto: PESCE,SALMONE,COHO (ARGENTO),CRUDO (NATIVO DELL'ALASKA), grassi: 5.57, carboidrati: 0.0, proteine: 22.56, calorie: 140, zuccheri: 0.0, fibre: 0.0
nome del prodotto: PESCE,SALMONE,ROSSO,(SOCKEYE),KIPPERED (NATIVO DELL'ALASKA), grassi: 4.75, carboidrati: 0.0, proteine: 24.5, calorie: 141, zuccheri: 0.0, fibre: 0.0
nome del prodotto: PESCE,SALMONE,REALE,CON PELLE,KIPPERED,(NATIVO DELL'ALASKA), grassi: 12.95, carboidrati: 0.0, proteine: 23.19, calorie: 209, zuccheri: 0.0, fibre: 0.0
```

I dati estratti dal database (quindi dal set di dati adottato) forniscono tre diversi campioni per il salmone, ma si può notare che il primo è **CRUDO** e, mentre Qwen lo sta usando, Llama di solito lo ignora.
